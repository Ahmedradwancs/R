...
Distance_matrix <- function(m1, m2) {
  n1 <- nrow(m1)
  n2 <- nrow(m2)
  output <- matrix(0, nrow = n1, ncol = n2)
  
  for (i in 1:n1) {
    for (j in 1:n2) {
      p1 <- unlist(m1[i, ])
      p2 <- unlist(m2[j, ])
      output[i, j] <- Distance(p1, p2)
    }
  }
...
Distance_matrix_result <- Distance_matrix(m1, m2)
